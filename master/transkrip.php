      ...
      <div class="container">
        <div class="card">
          <div class="card-body">
            ...
            $NIM = $_GET['NIM'];
            $sqll = "SELECT * FROM pendaftaran JOIN mahasiswa USING(NIM) WHERE NIM='$NIM'";
            $query = mysqli_query($conn, $sqll);
            $row = mysqli_num_rows($query);
            $result = mysqli_fetch_array($query);
            $content = $result['transkip_nilai'];
            ?>
            <object style="width: 100%; height: 1000px;" data="../transkip/<?= $content; ?>" type=""></object>
          </div>
        </div>
      </div>
      <!-- end -->
