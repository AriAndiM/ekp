...
$query1 = "SELECT * FROM pembimbing_lapangan WHERE NIM='$nim_mhs'";
$sql1 = mysqli_query($conn, $query1);
$cek_pembimbing_lapangan = mysqli_num_rows($sql1);

$query2 = "SELECT * FROM proposal WHERE NIM='$nim_mhs'";
$sql2 = mysqli_query($conn, $query2);
$cek_proposal = mysqli_num_rows($sql2);
if (empty($_SESSION['login'])) {
  header("location:../login_mahasiswa.php");
}
if (empty($cek_proposal)) {
  echo "<script>alert('Anda belum mengunggah proposal'); window.location='upload-proposal.php'</script>";
}
if (empty($cek_pembimbing_lapangan)) {
  echo "<script>alert('Anda belum memiliki pembimbing lapangan'); window.location='index-mahasiswa.php'</script>";
}
?>
      ...
      <main class="main-content col-lg-10 col-md-9 col-sm-12 p-0 offset-lg-2 offset-md-3">
        ...
        <div class="container-fluid">
          <div class="card" style="padding: -10px; height: fit-content; margin: 10%;">
            <h3 style="margin-top: 20px; margin-left: 20px; margin-bottom: 0px;">Upload Laporan</h3>
            <div style="outline: 3px dashed black; padding: 30px; border-radius: 15px; margin: 30px;">
              <form method="POST" action="insert-upload-laporan.php" enctype="multipart/form-data">
                <?php
                $nim = $_SESSION['NIM'];
                $query3 = "SELECT * FROM laporan WHERE NIM = '$nim'";
                $sql3 = mysqli_query($conn, $query3);
                $cek3 = mysqli_num_rows($sql3);
                // $query4 = "SELECT * FROM proposal WHERE NIM = '$nim' AND status = 'Disetujui'";
                ...
                <div style="text-align: center; ">
                  <input type="file" accept=".pdf" name="laporan" required <?php echo $read; ?>>
                  <button type="submit" style="margin: 10px;" class="btn btn-primary" name="submit" value="<?php echo $val; ?>" class="btn">submit</button>
                </div>
              </form>
            </div>
            <br>
            ...
            <div>
              <table class="table m-3">
                ...
                ?>
                  <tr>
                    <td><a style="color: grey;" href="view-laporan.php?NIM=<?php echo $row['NIM'] ?>"><?php echo $row['laporan']; ?></a></td>
                    <td style="text-align: center;"><?php echo $row['status']; ?></td>
                    <td style="text-align: center;"><a href="delete-laporan.php?id=<?php echo $row['id_laporan']; ?>"><?php echo $act; ?></a></td>
                  </tr>
                <?php } ?>
              </table>
            </div>
