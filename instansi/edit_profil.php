...
<head>
  ...
  <title>Update Profil Pembimbing Lapangan</title>
                  ...
                  <li class="list-group-item p-3">
                    <div class="row">
                      <div class="col">
                        <?php
                        $nama_pembimbing_lapangan = $_SESSION['nama_pembimbing_lapangan'];
                        $sql = mysqli_query($conn, "SELECT * FROM daftar_pembimbing WHERE nama_pembimbing_lapangan='$nama_pembimbing_lapangan'");
                        $row = mysqli_fetch_array($sql)
                        ?>
                        <form method="POST" action="action-edit-profil.php">
                          <input type="hidden" name="id_daftar_pembimbing" value="<?php echo $row["id_daftar_pembimbing"] ?>">
                          <input type="hidden" name="password_pembimbing_lapangan" value="<?php echo $row["password_pembimbing_lapangan"] ?>">
                          <div class="form-row">
                            <div class="form-group col-md-6">
                              <label>Nama</label>
                              <input type="text" class="form-control" name="nama_pembimbing_lapangan" required value="<?php echo $row['nama_pembimbing_lapangan'] ?>">
                            </div>
                            <div class="form-group col-md-6">
                              <label for="NIM">NIP</label>
                              <input type="text" class="form-control" name="NIP_pembimbing_lapangan" required value="<?php echo $row['NIP_pembimbing_lapangan'] ?>">
                            </div>
                          </div>
                          <div class="form-row">
                            <div class="form-group col-md-6">
                              <label for="email_pembimbing_lapangan">Email</label>
                              <input type="text" class="form-control" name="email_pembimbing_lapangan" required value="<?php echo $row['email_pembimbing_lapangan'] ?>">
                            </div>
                            <div class="form-group col-md-6">
                              <label for="inputState">Nama Instansi</label>
                              <select class="form-control" name="id_instansi" require>
                                <?php
                                ...
                                ?>
                                <option value="<?= $row1['id_instansi'] ?>"><?= $row1['nama_instansi'] ?></option>
                                <?php
